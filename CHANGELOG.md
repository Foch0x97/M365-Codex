...
一次全项目终审的产物。查出的问题里有几个的共同点是**"看起来在工作，其实没有"**——
测试全绿、界面正常、日志无异常，但实际行为和声称的不一致。

### 修复

- **CI 从管理界面进仓库那次提交起就一直是红的**：凭据扫描的排除模式
  `^\.env\.example$` 只锚定仓库根，把 `apps/web/.env.example` 判成了疑似凭据文件。
- **镜像里从来没有 `config/models.json`**：Dockerfile 没复制 `config/`，而
  `loadModels` 的 `catch` 里什么都不说，于是线上 `/v1/models` 一直只返回 1 个内置模型
  （配置文件里有 3 个）。现在镜像带上该目录、降级必须回调说明原因、CI 断言条数一致。
- **`NO_PROXY` 配了不生效**：被解析、被展示在设置页、文档说"对上游与 OAuth 生效"，
  但没有任何地方拿它做排除判断。现已在上游 WebSocket 与 OAuth 两条路径上真正生效，
  支持 `*`、裸域名匹配子域、`.`/`*.` 前缀、端口与大小写，并覆盖
  `notexample.com` 不该被 `example.com` 命中这类边界。
- **管理界面向管理员承诺 debug 日志模式"会自动过期恢复 strict"，而服务端没有这个机制**：
  切过去会无限期停在 debug。现在两头都补齐——服务端真的做了自动过期
  （`LOG_PRIVACY_DEBUG_TTL_MS`，默认 1 小时，到期恢复 strict 并写审计日志，
  且 `log_privacy_mode` 改为热生效，否则"自动过期"只会改数据库而不影响运行中的进程），
  界面文案同时改成如实描述。
- **SIGTERM 没做到文档承诺的事**：只被动排空，不主动关闭上游连接、不落库在途状态，
  一旦排空超时被 SIGKILL 就什么都没留下。现在关闭时取消在途请求并把
  `in_progress` 落为 `incomplete`，与重启恢复共用同一套处置逻辑。
- **前后端两处字段名对不上**（代理批量导入、文件清理），真实联调会显示 `undefined`。
- **`POST /admin/backup` 忽略 `includeFiles`**，且带了非法 body 会被静默吞掉——
  传 `{"includeFiles":"false"}` 会照样生成完整包而调用方以为只含数据库。现已接线，
  并区分"没带 body"与"带了但不合法"（后者返回 400）。
- **tar 互操作测试时绿时红**：传 Windows 绝对路径时 MSYS 版 GNU tar 把 `C:`
  当成远程主机语法。连着被两轮审查误报成"环境问题"。

### 新增

- API Key 补齐计划 §10.1 里一直悬空的四项：备注、累计请求次数、按 Key 的工具调用上限
  与上传大小上限（后两者真实生效，且不得突破全局天花板）。
- 管理界面的备份与恢复页真正实现（生成/列表/下载/上传恢复/诊断包），
  恢复明确提示"校验通过只代表已落盘，必须重启后才生效"。
- 发布链路补上门禁：`release.yml` 自带类型/Lint/测试/audit/凭据扫描，`publish` 依赖它；
  `latest` 收紧为只跟正式 Release（原来任意 `v*` 裸标签都会移动它）；
  `publish` 声明 `environment: release`；新增 `.github/dependabot.yml`。

### 变更

- 清理死代码：服务端 1 处孤立类型、前端 3 处、探针 3 处。
  （服务端几乎没有可删的——`noUnusedLocals` 与 ESLint 早已把私有层面清零。）
- 文档纠正：README 与部署文档此前仍称协议"未经真实流量校准"，且举的例子正是
  已被实测证伪的 `arguments[0].messages[]` 数组形态。
...
里程碑 **M5 · 工具调用与完整代理循环** 完成。模型请求调用工具 → Codex 本机执行 →
结果回传 → 继续推理 → 完成，整条链路打通。

> 工具由 **Codex 客户端**执行，本网关只负责把上游的调用决策翻译成 Responses
> `function_call` 并把结果送回上游（见实施计划 §7.1）。

### 新增

- **严格工具协议**：解析请求里的 `tools`（扁平与嵌套两种写法），用 JSON Schema 校验
  上游产出的参数；参数不合法时向上游请求修复，最多两次。
- **只调声明过的工具**：未声明的工具与非法 JSON 参数，修复无果后**绝不发给客户端**，
  整轮如实报错；只是不符合参数 schema 的会如实发出并记录告警。
- **`function_call_output` 回传**：匹配已发出的 `call_id`、幂等标记完成、携结果续推理；
  不匹配任何已发出调用的 `call_id` 直接返回 400，不动上游。
- **工具调用持久化**（`tool_calls` 表）：`UNIQUE (response_id, call_id)` +
  `emitted → completed` 单向流转，保证 SSE 重连或重复提交都不会让同一次调用被执行两次。
- **副作用不跨账号重放**：携带工具结果的请求一旦失败即止，不换账号重发，避免同一副作用
  在两个账号上各执行一次。
- **两条工具通道**（`TOOLS_MODE=native|prompt|auto`）：原生结构化声明，或提示词约束
  `<tool_call>{…}</tool_call>` 并从正文流式剥离——工具 JSON 不会同时作为正文重复输出。
- **代理循环上限走配置**（`TOOLS_MAX_*`、`TOOLS_ALLOW_PARALLEL`）：每轮调用数、工具轮次、
  累计调用数、结果大小、参数修复次数、是否允许并行。超限返回明确错误，不静默截断。

### 变更

- 工具协议代码按实施计划 §2 的目录结构落到 `apps/server/src/tools/`。
- OpenAPI 契约补上 `function_call` 输出项与 `tool_choice` / `parallel_tool_calls` 请求字段。
